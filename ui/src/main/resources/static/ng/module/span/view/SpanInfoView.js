                ...
                if (resp.code == 0) {
                    if (resp.data != null && resp.data != undefined) {
                        ...
                        for (var i = 0; i < resp.data.slaveThread.length; i++) {
                            ...
                            view.$el.find('.slave').append("<div class='panel panel-default'><div class='panel-heading'>异步TASK_"+(i+1)+"</div><div class='panel-body'>" + view.model.htm + "</div></div>");
                            view.model.htm = "";//清理
                        }
                    } else {
        ...
        makeTree: function (treeNode) {
            ...
            if (treeNode != null && treeNode.node != null) {
                ...
                if(treeNode.node.merge > 1){
                    view.model.htm += treeNode.node.short_title + "(重复执行:<span style='color:#12c58c'>" + treeNode.node.merge + "</span>次,平均耗时:<span style='color:#ff6fb7'>" + treeNode.node.avg_duration + "ms</span>,最小耗时:<span style='color:#ff6fb7'>" + treeNode.node.min_duration + "ms</span>,最大耗时:<span style='color:#ff6fb7'>" + treeNode.node.max_duration + "ms</span>)</a>";
                }else{
